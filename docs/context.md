...
- lib/providers/chat_provider.dart: Manages chat UI state, message history, and server session continuity (conversationId).
...
- DatabaseService: Relational storage for transactions and wallet data.
- AppConfig: Centralized source of truth for URLs and API keys.
- ChatProvider: Hybrid state manager for UI and server-side session persistence.
