...
1. **Test Execution**: First check if `.gemini/scripts/test-and-log.sh` exists. If it does, use it for log capture. If not, read `.gemini/testing-config.md` for the test command, or fall back to the project's native test command (see Error Recovery below).
...
2. **Test Execution**:
   ...
   if [ -x ".gemini/scripts/test-and-log.sh" ]; then
     .gemini/scripts/test-and-log.sh tests/[test_file]
   # Option B: Use testing-config.md command
   elif [ -f ".gemini/testing-config.md" ]; then
     # Read test_command from config frontmatter and run it
     test_cmd=$(grep '^test_command:' .gemini/testing-config.md | sed 's/^test_command: *//')
     $test_cmd
