...
[ -d ".gemini" ] && echo "  ✅ .gemini directory exists" || { echo "  ❌ .gemini directory missing"; ((errors++)); }
[ -d ".gemini/prds" ] && echo "  ✅ PRDs directory exists" || echo "  ⚠️ PRDs directory missing"
[ -d ".gemini/epics" ] && echo "  ✅ Epics directory exists" || echo "  ⚠️ Epics directory missing"
[ -d ".gemini/rules" ] && echo "  ✅ Rules directory exists" || echo "  ⚠️ Rules directory missing"
echo ""
...
orphaned=$(find .gemini -name "[0-9]*.md" -not -path ".gemini/epics/*/*" 2>/dev/null | wc -l)
...
for task_file in .gemini/epics/*/[0-9]*.md; do
...
for file in $(find .gemini -name "*.md" -path "*/epics/*" -o -path "*/prds/*" 2>/dev/null); do
  if ! grep -q "^---" "$file"; then
