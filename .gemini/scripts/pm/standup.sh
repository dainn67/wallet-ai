...
recent_files=$(find .gemini -name "*.md" -mtime -1 2>/dev/null)
...
for updates_dir in .gemini/epics/*/updates/*/; do
...
echo ""
echo "📊 Quick Stats:"
total_tasks=$(find .gemini/epics -name "[0-9]*.md" 2>/dev/null | wc -l)
open_tasks=$(find .gemini/epics -name "[0-9]*.md" -exec grep -l "^status: *open" {} \; 2>/dev/null | wc -l)
closed_tasks=$(find .gemini/epics -name "[0-9]*.md" -exec grep -l "^status: *closed" {} \; 2>/dev/null | wc -l)
