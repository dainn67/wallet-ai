...
# Process all markdown files in .gemini directory
...
done < <(find .gemini/ -name "*.md" -type f -print0 2>/dev/null)
...
if [ $files_modified -gt 0 ]; then
    ...
    echo "./.gemini/scripts/check-path-standards.sh"
...
if [ $files_modified -gt 0 ]; then
    ...
    echo "find .gemini/ -name '*.backup' -delete"
fi
