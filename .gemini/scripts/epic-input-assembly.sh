...
HANDOFF_DIR="$PROJECT_ROOT/.gemini/context/handoffs"
{
...
EPIC_CTX="$PROJECT_ROOT/.gemini/context/epics/${EPIC_NAME}.md"
{
  ...
  if [ -f "$EPIC_CTX" ]; then
    ...
    echo "No epic context file found at .gemini/context/epics/${EPIC_NAME}.md"
  fi
...
ARCH_FILE="$PROJECT_ROOT/.gemini/context/architecture-decisions.md"
{
  ...
  if [ -f "$ARCH_FILE" ]; then
    ...
    EPIC_FILE="$PROJECT_ROOT/.gemini/epics/$EPIC_NAME/epic.md"
    if [ -f "$EPIC_FILE" ]; then
      python3 -c "
import re, sys
...
EPIC_CREATED=$(grep "^created:" "$PROJECT_ROOT/.gemini/epics/$EPIC_NAME/epic.md" 2>/dev/null \
...
echo "── 10. Codebase Structure ──"
{
  ...
  if command -v tree >/dev/null 2>&1; then
    tree -L 3 -I 'node_modules|.git|__pycache__|.venv|venv|dist|build|.gemini' \
...
IFACE_FILE="$PROJECT_ROOT/.gemini/context/active-interfaces.md"
{
...
# ── 13. Known Issues ──
KNOWN_FILE="$PROJECT_ROOT/.gemini/context/known-issues.md"
...
REPORTS_DIR="$PROJECT_ROOT/.gemini/context/verify/epic-reports"
