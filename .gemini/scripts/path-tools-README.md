...
### 1. Validation Script
```bash
./.gemini/scripts/check-path-standards.sh
```
...
### 2. Fix Script  
```bash
./.gemini/scripts/fix-path-standards.sh
```
...
- name: Path Standards Check
  run: ./.gemini/scripts/check-path-standards.sh
```

### Clean Up Backups
After confirming fixes are correct:
```bash
find .gemini/ -name '*.backup' -delete
```

## Standards Reference
For detailed path usage guidelines, see: `.gemini/rules/path-standards.md`