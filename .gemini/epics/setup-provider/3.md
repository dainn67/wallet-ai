...
## Acceptance Criteria
- [x] **FR-2 / Happy path:** `lib/providers/` directory is created.
- [x] **FR-4 / Happy path:** `CounterProvider` class implements `ChangeNotifier` and has an `int _count` variable and an `increment()` method.
- [x] **FR-4 / Scenario:** `notifyListeners()` is called inside `increment()`.
