...
```bash
test -f .gemini/context/handoffs/latest.md && find .gemini/context/handoffs/latest.md -mmin -10 -print | grep -q .
```
...
Read `.gemini/context/verify/state.json`. If active task with verify_mode=STRICT and no VERIFY_PASS:
...
Your work is preserved in:
  - .gemini/context/handoffs/latest.md (handoff note)
  - .gemini/context/verify/results/ (verify logs)
