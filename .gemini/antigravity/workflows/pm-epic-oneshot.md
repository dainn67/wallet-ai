...
2. **Locate epic:**
   - If `.gemini/epics/$EPIC_NAME/epic.md` doesn't exist → `❌ Epic not found. Run: /pm:prd-parse $EPIC_NAME` and stop.

3. **Check existing tasks:**
   - If task files (`[0-9]*.md`) exist in `.gemini/epics/$EPIC_NAME/`:
     ```
...
Tasks are saved locally in .gemini/epics/$EPIC_NAME/
