...
gh issue view $ISSUE_NUMBER --repo "$REPO" --json state -q .state || { echo "❌ Issue #$ISSUE_NUMBER not found on GitHub."; exit 1; }

# 3. Check local updates exist
updates_dir=$(find .gemini/epics/*/updates/$ISSUE_NUMBER -type d 2>/dev/null | head -1)
[ -z "$updates_dir" ] && { echo "❌ No local updates for #$ISSUE_NUMBER. Run: /pm:issue-start $ISSUE_NUMBER"; exit 1; }
...
if [ -f .gemini/context/verify/state.json ]; then
  active=$(jq -r '.active_task // empty' .gemini/context/verify/state.json 2>/dev/null)
  if [ -n "$active" ] && [ "$active" != "null" ]; then
    if [ ! -f .gemini/context/handoffs/latest.md ] || [ -z "$(find .gemini/context/handoffs/latest.md -mmin -10 -print 2>/dev/null)" ]; then
...
```bash
git log --oneline --grep="Issue #$ISSUE_NUMBER" -10 2>/dev/null
```
...
```bash
gh issue comment $ISSUE_NUMBER --repo "$REPO" --body-file /tmp/issue-sync-comment.md
```
...
```
✅ Synced to GitHub Issue #$ISSUE_NUMBER
  Comment posted with {item_count} updates
  Progress: {completion}%

Next steps:
  - View: gh issue view $ISSUE_NUMBER --comments
  - Complete when done: /pm:issue-complete $ISSUE_NUMBER
```
