...
1. **Find local task file:**
   - First check if `.gemini/epics/*/$ISSUE_NUMBER.md` exists (new naming convention)
   - If not found, search for file containing `github:.*issues/$ISSUE_NUMBER` in frontmatter (old naming)
   - If not found: "❌ No local task for issue #$ISSUE_NUMBER. Run: /pm:import first"

2. **Check for existing analysis:**
   ```bash
   test -f .gemini/epics/*/$ISSUE_NUMBER-analysis.md && echo "⚠️ Analysis already exists. Overwrite? (yes/no)"
   ```
...
Create `.gemini/epics/{epic_name}/$ISSUE_NUMBER-analysis.md`:
...
# Parallel Work Analysis: Issue #$ISSUE_NUMBER
