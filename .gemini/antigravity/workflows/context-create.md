---
name: pm-create
description: Create Initial Context
---

# Create Initial Context

This command creates the initial project context documentation in `.gemini/context/` by analyzing the current project state and establishing comprehensive baseline documentation.

## Required Rules

**IMPORTANT:** Before executing this command, read and follow:
- `.gemini/rules/frontmatter.md` - For getting real current date/time
...
- Run: `ls -la .gemini/context/ 2>/dev/null`
- If directory exists and has files:
  - Count existing files: `ls -1 .gemini/context/*.md 2>/dev/null | wc -l`
...
- If `.gemini/` doesn't exist, create it: `mkdir -p .gemini/context/`
- Verify write permissions: `touch .gemini/context/.test && rm .gemini/context/.test`
...
**Common Issues:**
- **No write permissions:** "❌ Cannot write to .gemini/context/. Check permissions."
...
📁 Created context in: .gemini/context/
...
After creating all context files, regenerate the project's README.md:

1. **Read context files** for source data:
   - `.gemini/context/project-overview.md` — features, capabilities
   - `.gemini/context/project-brief.md` — scope, goals, success criteria
   - `.gemini/context/tech-context.md` — tech stack, dependencies, setup
   - `.gemini/context/project-structure.md` — directory layout
   - `.gemini/context/product-context.md` — user personas, use cases

2. **Read existing README.md** (if exists):
   - Identify manually-maintained sections to preserve (Contributing, License, Roadmap, etc.)
   - Note the current structure and tone

3. **Generate README.md** with these sections:
   - Project name and description (from project-brief + project-overview)
   - Features / What it does (from project-overview + product-context)
   - Tech stack (from tech-context)
   - Getting Started / Installation (from tech-context)
   - Project Structure (from project-structure)
   - [Preserve any existing manually-maintained sections]

4. **Write** the updated README.md to project root
5. **Display**: `📝 README.md regenerated from context files`

## Context Gathering Commands

Use these commands to gather project information:
- Target directory: `.gemini/context/` (create if needed)
