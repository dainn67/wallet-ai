...
If profile doesn't exist, fall back to `.gemini/context/verify/profiles/generic.sh`.
...
Append the run to `.gemini/context/verify/results/task-{issue_number}-verify.log` with timestamp.
...
if command -v ace_feature_enabled &>/dev/null && ace_feature_enabled "reflection" 2>/dev/null; then
  ...
  if [ -n "$epic" ] && [ -n "$issue" ]; then
    # Generate reflection
    source .gemini/scripts/pm/reflection-generate.sh 2>/dev/null || true
...
```
Show: "⚠️ Task marked needs-attention. Review: .gemini/epics/{epic}/reflections/"
