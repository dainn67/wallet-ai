...
task_count=$(ls .gemini/epics/$EPIC_NAME/[0-9]*.md 2>/dev/null | grep -cv epic.md)
[ "$task_count" -eq 0 ] && { echo "❌ No tasks to sync. Run: /pm:epic-decompose $EPIC_NAME"; exit 1; }

# 3. Check if already synced (idempotency)
if [ -f ".gemini/epics/$EPIC_NAME/github-mapping.md" ]; then
  echo "⚠️ Epic already synced to GitHub. Mapping file exists."
  echo "To re-sync, first delete: .gemini/epics/$EPIC_NAME/github-mapping.md"
  exit 0
fi
...
sed '1,/^---$/d; 1,/^---$/d' .gemini/epics/$EPIC_NAME/epic.md > /tmp/epic-body.md
...
```bash
epic_file=".gemini/epics/$EPIC_NAME/epic.md"
...
Read `.gemini/epics/$EPIC_NAME/epic.md` with the Read tool. Then use the Write tool to update:
...
✅ Synced to GitHub
  ...
  - Mapping: .gemini/epics/$EPIC_NAME/github-mapping.md
