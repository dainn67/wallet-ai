...
2. **Locate PRD:**
   - If `.gemini/prds/$FEATURE_NAME.md` doesn't exist → `❌ PRD not found: .gemini/prds/$FEATURE_NAME.md. Run: /pm:prd-new $FEATURE_NAME` and stop.
3. **PRD quality gate:**
...
4. **Check existing epic:**
   - If `.gemini/epics/$FEATURE_NAME/epic.md` exists → ask `⚠️ Epic '$FEATURE_NAME' already exists. Overwrite? (yes/no)`
   - If no → stop.
...
6. **Ensure directory:** `mkdir -p .gemini/epics/$FEATURE_NAME 2>/dev/null`
...
Load `.gemini/prds/$FEATURE_NAME.md` and perform deep analysis:
...
Create: `.gemini/epics/$FEATURE_NAME/epic.md`
...
prd: .gemini/prds/$FEATURE_NAME.md
...
1. **Confirm:** `✅ Epic created: .gemini/epics/$FEATURE_NAME/epic.md`
