...
2. **Locate PRD:**
   - If `.gemini/prds/$FEATURE_NAME.md` doesn't exist → `❌ PRD not found: .gemini/prds/$FEATURE_NAME.md` and stop.
3. **Ensure directory:** `mkdir -p .gemini/prds 2>/dev/null`
...
- `.gemini/prds/$FEATURE_NAME.md` — the PRD to validate
- `.gemini/context/tech-context.md` — for constraint validation
- `.gemini/context/product-context.md` — for persona validation
...
Save to `.gemini/prds/.validation-$FEATURE_NAME.md`:
...
📋 Validation: $FEATURE_NAME — [STATUS]
  ...
  Report: .gemini/prds/.validation-$FEATURE_NAME.md
