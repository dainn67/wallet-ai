...
2. **Check for active agents:**
   Read `.gemini/epics/$EPIC_NAME/execution-status.md`
...
Update `.gemini/epics/$EPIC_NAME/epic.md`:
...
feature_list=""
if [ -d ".gemini/epics/$EPIC_NAME" ]; then
  cd .gemini/epics/$EPIC_NAME
...
epic_github_line=$(grep 'github:' .gemini/epics/$EPIC_NAME/epic.md 2>/dev/null || true)
...
mv .gemini/epics/$EPIC_NAME .gemini/epics/.archived/
echo "✅ Epic archived: .gemini/epics/.archived/$EPIC_NAME"
```
...
REPO=""
epic_github=$(grep '^github:' .gemini/epics/.archived/$EPIC_NAME/epic.md 2>/dev/null | head -1 | sed 's/^github: *//')
...
# Extract epic issue number
epic_github_line=$(grep 'github:' .gemini/epics/.archived/$EPIC_NAME/epic.md 2>/dev/null || true)
if [ -n "$epic_github_line" ]; then
  epic_issue=$(echo "$epic_github_line" | grep -oE '[0-9]+$' || true)
else
  epic_issue=""
...
# Close task issues
for task_file in .gemini/epics/.archived/$EPIC_NAME/[0-9]*.md; do
  [ -f "$task_file" ] || continue
