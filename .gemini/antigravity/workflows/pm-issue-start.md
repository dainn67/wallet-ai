...
2. **Get issue details:**
   ...
   **If IS_LIGHT_PATH=true — Create debug journal:**
   ...
   ```
   Then create `.gemini/context/sessions/issue-$ISSUE_NUMBER-debug.md`:
   ```markdown
...
3. **Find local task file:**
   ...
   - First check if `.gemini/epics/*/$ISSUE_NUMBER.md` exists (new naming)
...
6. **Check for analysis:**
   ...
   ```bash
   test -f .gemini/epics/*/$ISSUE_NUMBER-analysis.md || echo "❌ No analysis found for issue #$ISSUE_NUMBER
...
Read `.gemini/epics/{epic_name}/$ISSUE_NUMBER-analysis.md`:
...
```bash
mkdir -p .gemini/epics/{epic_name}/updates/$ISSUE_NUMBER
```
...
Create `.gemini/epics/{epic_name}/updates/$ISSUE_NUMBER/stream-{X}.md`:
```markdown
---
issue: $ISSUE_NUMBER
stream: {stream_name}
...
Task:
  ...
  prompt: |
    ...
    4. Update progress in: .gemini/epics/{epic_name}/updates/$ISSUE_NUMBER/stream-{X}.md
...
Debug journal: .gemini/context/sessions/issue-$ISSUE_NUMBER-debug.md (mode: {mode})
...
Progress tracking:
  .gemini/epics/{epic_name}/updates/$ISSUE_NUMBER/
