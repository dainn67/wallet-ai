...
2. **Check existing PRD:**
   - If `.gemini/prds/$FEATURE_NAME.md` exists → ask `⚠️ PRD '$FEATURE_NAME' already exists. Overwrite? (yes/no)`
   - If user says no → stop.
3. **Check draft resume:**
   - If `.gemini/prds/.draft-$FEATURE_NAME.md` exists → ask `📝 Found unfinished draft for '$FEATURE_NAME'. Resume where you left off? (yes/no)`
...
→ Ask these questions then STOP. When user responds, save checkpoint to `.gemini/prds/.draft-$FEATURE_NAME.md` with discovery notes, then proceed to Wave 2.
...
Save the completed PRD to: `.gemini/prds/$FEATURE_NAME.md`
...
1. **Clean up:** Delete draft file if exists: `rm -f .gemini/prds/.draft-$FEATURE_NAME.md`
2. **Confirm:** `✅ PRD created: .gemini/prds/$FEATURE_NAME.md`
