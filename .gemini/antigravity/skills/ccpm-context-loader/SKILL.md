...
Load context from `.gemini/context/` before starting any implementation work.
...
3. Review epic context from `.gemini/context/epics/{name}.md` if the file exists
4. Review `.gemini/context/verify/state.json` for current verification state (pass/fail, iterations)
