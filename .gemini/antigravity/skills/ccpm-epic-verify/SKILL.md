...
The script will:
- Assemble epic context via `.gemini/scripts/epic-input-assembly.sh` if available
- Load review criteria from `.gemini/prompts/epic-phase-a.md`
- Read current epic state from `.gemini/context/verify/epic-state.json`
...
The script will:
- Run `.gemini/context/verify/epic-verify.sh` if available, or fall back to generic profiles
...
- Max iterations is read from `.gemini/config/epic-verify.json` (default: 30)
