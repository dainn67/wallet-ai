...
# If not provided, reads from .gemini/context/verify/state.json

set -uo pipefail

VERIFY_STATE=".gemini/context/verify/state.json"
...
if [ -z "$EPIC" ] || [ -z "$TASK_N" ]; then
  ...
  echo ""
  echo "→ Provide epic name and task number, or ensure .gemini/context/verify/state.json is populated."
  echo ""
  exit 0
fi

DESIGN_FILE=".gemini/epics/${EPIC}/designs/task-${TASK_N}-design.md"
