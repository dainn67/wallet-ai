...
if [ ! -f "$LATEST" ]; then
  echo "❌ Handoff note missing: .gemini/context/handoffs/latest.md"
  echo "   Write a handoff note before completing this task."
  echo "   Template: .gemini/context/handoffs/TEMPLATE.md"
  FAIL=1
else
...
if [ "$FAIL" -eq 0 ]; then
  ...
  if [ -z "$fresh" ]; then
    ...
    echo "   Update .gemini/context/handoffs/latest.md with current task results."
    FAIL=1
  else
...
if [ "$FAIL" -ne 0 ]; then
  ...
  echo ""
  echo "Quick fix:"
  echo "  1. Copy template: cp .gemini/context/handoffs/TEMPLATE.md .gemini/context/handoffs/latest.md"
