#!/bin/sh
# POSIX-compliant pre-tool-use hook for Bash tool (Gemini CLI PreToolUse API)
#
...
#   - To modify: exit 0 + JSON with hookSpecificOutput.updatedInput
#   - Exit 0 with no output = allow unchanged
#   - Exit 2 = block tool call

DEBUG_MODE="${GEMINI_HOOK_DEBUG:-false}"
...
## JSON output helper — emits updatedInput for PreToolUse
emit_updated_command() {
    new_cmd=$1
    # Escape JSON special chars in command
    escaped_cmd=$(printf '%s' "$new_cmd" | sed 's/\\/\\\\/g; s/"/\\"/g; s/	/\\t/g')
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","updatedInput":{"command":"%s"}}}\n' "$escaped_cmd"
}

main() {
    # Read JSON from stdin (Gemini CLI hooks API)
    input=$(cat)

    # Extract command from JSON
    original_command=""
    if command -v jq >/dev/null 2>&1; then
        original_command=$(printf '%s' "$input" | jq -r '.tool_input.command // empty' 2>/dev/null)
    else
        original_command=$(printf '%s' "$input" | python3 -c "
import json, sys
...
" 2>/dev/null)
    ...
    if [ -z "$original_command" ]; then
        debug_log "No command found in stdin, passing through"
        exit 0
    fi
    ...
    emit_updated_command "${modified_command}"
}
