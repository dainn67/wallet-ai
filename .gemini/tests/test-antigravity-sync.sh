...
Run `bash .gemini/scripts/pm/status.sh` and display the output to the user.
CMD
...
## Instructions
1. Read epic from `.gemini/epics/$ARGUMENTS/epic.md`
...
test_detect_gaps() {
  ...
  touch "$root/antigravity/workflows/pm-status.md"
  touch "$root/antigravity/workflows/pm-epic-start.md"
  touch "$root/antigravity/rules/ccpm-frontmatter.md"
  touch "$root/antigravity/rules/ccpm-git-workflows.md"
