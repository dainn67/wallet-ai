...
cleanup() {
  ...
  rm -rf ".gemini/epics/test-epic/designs"
...
output=$(echo '{"tool_name":"Write","tool_input":{"file_path":".gemini/epics/test-epic/designs/task-99-design.md","content":"design"}}' | bash "$PROJECT_ROOT/hooks/pre-tool-use.sh" "$CCPM_ROOT" 2>&1)
exit_code=$?
...
# PreToolUse checks ".gemini/epics/..." (relative), PostTask checks "$CCPM_ROOT/epics/..."
# At runtime both resolve to .gemini/epics/, but in test CCPM_ROOT=project root
mkdir -p ".gemini/epics/test-epic/designs"
mkdir -p "$CCPM_ROOT/epics/test-epic/designs"
echo "# Design" > ".gemini/epics/test-epic/designs/task-99-design.md"
...
echo '{"name": "superpowers-plugin", "version": "4.1.1"}' > .gemini-plugin/plugin.json
output=$(bash "$PROJECT_ROOT/hooks/pre-task.sh" "$CCPM_ROOT" 2>&1)
