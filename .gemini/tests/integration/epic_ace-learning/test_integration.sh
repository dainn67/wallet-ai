...
ORIG_SKILLBOOK=""
if [ -f ".gemini/context/skillbook.md" ]; then
  ORIG_SKILLBOOK=$(cat .gemini/context/skillbook.md)
fi
cleanup() {
  # Restore original skillbook
  if [ -n "$ORIG_SKILLBOOK" ]; then
    echo "$ORIG_SKILLBOOK" > .gemini/context/skillbook.md
  fi
  # Clean up test fixtures
  rm -rf .gemini/epics/integ-test-$$ 2>/dev/null
}
trap cleanup EXIT
...
entry=$(tail -20 .gemini/context/skillbook.md)
...
low_threshold=$(python3 -c "import json; c=json.load(open('.gemini/config/ace-learning.json')); print(c['complexity']['thresholds']['low'])" 2>/dev/null)
high_threshold=$(python3 -c "import json; c=json.load(open('.gemini/config/ace-learning.json')); print(c['complexity']['thresholds']['high'])" 2>/dev/null)
...
assert_contains "$(cat .gemini/config/model-tiers.json)" "haiku\|sonnet\|opus" "Model tiers defined"
...
source .gemini/scripts/pm/lifecycle-helpers.sh 2>/dev/null
lines_before=$(wc -l < .gemini/context/ace-learning-log.md 2>/dev/null || echo "0")
ace_log "TEST" "integration test entry" 2>/dev/null
lines_after=$(wc -l < .gemini/context/ace-learning-log.md 2>/dev/null || echo "0")
...
last_line=$(tail -1 .gemini/context/ace-learning-log.md)
...
issue_start=$(cat .gemini/commands/pm/issue-start.md)
issue_complete=$(cat .gemini/commands/pm/issue-complete.md)
verify_run=$(cat .gemini/commands/pm/verify-run.md)
