...
# ── Integration: Skills ↔ .gemini/scripts ──

def test_load_context_references_correct_paths():
    """ccpm-context-loader/load-context.sh references .gemini/ paths"""
    script = "antigravity/skills/ccpm-context-loader/scripts/load-context.sh"
    result = run(f"grep -c '.gemini/' '{script}'")
    count = int(result.stdout.strip() or "0")
    assert count >= 2, f"Expected ≥2 .gemini/ references, got {count}"
...
def test_check_design_uses_correct_path_pattern():
    """check-design.sh checks .gemini/epics/{epic}/designs/task-{N}-design.md"""
...
def test_plan_gaps_reads_epic_reports():
    """plan-gaps.sh reads from .gemini/context/verify/epic-reports/"""
...
def test_install_copies_active_ide_template():
    """Install copies active-ide.json template to .gemini/sync/"""
