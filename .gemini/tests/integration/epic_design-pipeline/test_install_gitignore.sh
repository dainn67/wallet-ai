...
# --- Test 1: Fresh install adds .gemini/designs/ to .gitignore ---
echo ""
echo "Test 1: Fresh install"
TMPDIR_TEST=$(mktemp -d)
bash "$INSTALL_SCRIPT" "$TMPDIR_TEST" >/dev/null 2>&1
has_entry=$(grep -cxF '.gemini/designs/' "$TMPDIR_TEST/.gitignore" 2>/dev/null || echo 0)
assert_ok ".gitignore contains .gemini/designs/ after install" "$([ "$has_entry" -ge 1 ] && echo true || echo false)"
rm -rf "$TMPDIR_TEST"
...
bash "$INSTALL_SCRIPT" "$TMPDIR_TEST" >/dev/null 2>&1
bash "$INSTALL_SCRIPT" "$TMPDIR_TEST" >/dev/null 2>&1
count=$(grep -cxF '.gemini/designs/' "$TMPDIR_TEST/.gitignore" 2>/dev/null || echo 0)
assert_ok ".gemini/designs/ appears exactly once after two installs (got $count)" "$([ "$count" = "1" ] && echo true || echo false)"
rm -rf "$TMPDIR_TEST"
...
has_designs=$(grep -cxF '.gemini/designs/' "$TMPDIR_TEST/.gitignore" 2>/dev/null || echo 0)
...
assert_ok ".gemini/designs/ added" "$([ "$has_designs" -ge 1 ] && echo true || echo false)"
rm -rf "$TMPDIR_TEST"
