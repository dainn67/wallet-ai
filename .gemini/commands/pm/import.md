...
if [[ "$ARGUMENTS" == *"--label"* ]]; then
  gh issue list --repo "$REPO" --label "{label}" --limit 1000 --json number,title,body,state,labels,createdAt,updatedAt
else
  gh issue list --repo "$REPO" --limit 1000 --json number,title,body,state,labels,createdAt,updatedAt
fi
```
...
**If Epic:**
```bash
mkdir -p .gemini/epics/{epic_name}
