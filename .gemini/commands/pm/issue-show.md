...
### 1. Fetch Issue Data
- Use `gh issue view $ARGUMENTS --repo "$REPO"` to get GitHub issue details
- Look for local task file: first check `.gemini/epics/*/$ARGUMENTS.md` (new naming)
...
```
📁 Local Files:
   Task file: .gemini/epics/{epic_name}/{task_file}
   Updates: .gemini/epics/{epic_name}/updates/$ARGUMENTS/
