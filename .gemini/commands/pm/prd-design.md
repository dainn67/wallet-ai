---
model: opus
allowed-tools: Bash, Read, Write, LS, Glob, Grep, AskUserQuestion
---
...
1. **Parse arguments:**
   ...
   - If `$ARGUMENTS` is empty → `❌ Missing feature name. Usage: /pm:prd-design <feature_name> [--screen <screen_name>]` and stop.
...
4. **Detect tools — determine operation mode:**
   ...
   .gemini/scripts/pm/detect-uupm.sh >/dev/null 2>&1 && uupm_available=true
   .gemini/scripts/pm/detect-stitch.sh >/dev/null 2>&1 && stitch_available=true
   ```
   Determine MODE:
...
6. **Check existing designs (re-run menu):**
   - If `.gemini/designs/$FEATURE/` exists and `TARGET_SCREEN` is empty:
     - Ask user:
       ...
       ```
     - Option 1: `rm -rf .gemini/designs/$FEATURE` then continue with full pipeline.
...
7. **Create directory structure:**
   ```bash
   mkdir -p .gemini/designs/$FEATURE/{prompts,screens,specs}
   ```
...
Generate `.gemini/designs/$FEATURE/design-system.md` with the following sections:
...
- **FULL or DESIGN_ONLY:** Invoke the UUPM skill to provide design intelligence. Use its recommendations as the foundation for the design system. Prompt UUPM with: product type, personas, mood keywords, and any brand constraints. Incorporate its output into the design system structure above. If UUPM invocation fails → fall back to Gemini native reasoning (same as TEXT_ONLY path).
- **MOCKUP_ONLY or TEXT_ONLY:** Use Gemini's native reasoning to generate the design system. Base decisions on: product type conventions, persona needs, mood keywords, and established design principles.

Write output to `.gemini/designs/$FEATURE/design-system.md`.
...
Create `.gemini/designs/$FEATURE/screen-inventory.md`:
...
For each screen, create `.gemini/designs/$FEATURE/prompts/{screen-name}.txt`:
...
- **FULL or MOCKUP_ONLY:** For each screen (in priority order):
  ...
  3. Save returned HTML to `.gemini/designs/$FEATURE/screens/{screen-name}.html`.
...
- **DESIGN_ONLY or TEXT_ONLY:**
  - Display: `ℹ️ Stitch MCP not available — prompts saved for manual use at .gemini/designs/$FEATURE/prompts/`
  - Display: `Tip: Paste prompts into stitch.withgoogle.com and save HTML to .gemini/designs/$FEATURE/screens/`
...
- If `.gemini/designs/$FEATURE/screens/{screen-name}.html` exists → parse HTML for component extraction.
- If no HTML → generate text-based spec from design system + screen description.

**3b. Generate spec:**

Write `.gemini/designs/$FEATURE/specs/{screen-name}-spec.md`:
