...
2. **Locate epic:**
   - If `.gemini/epics/$ARGUMENTS/epic.md` doesn't exist → `❌ Epic not found: .gemini/epics/$ARGUMENTS/epic.md. Run: /pm:prd-parse $ARGUMENTS` and stop.
3. **Epic quality gate:**
...
4. **Check existing tasks:**
   - Scan `.gemini/epics/$ARGUMENTS/` for task files (`[0-9]*.md`).
...
Check if `.gemini/designs/$ARGUMENTS/specs/` directory exists and contains `.md` files:
- If yes: build a map of screen-name to spec-file-path (e.g., `dashboard` → `.gemini/designs/$ARGUMENTS/specs/dashboard-spec.md`). This map will be used in Step 5 to enrich UI-related tasks.
...
Load `.gemini/epics/$ARGUMENTS/epic.md` and extract:
...
For each task, create `.gemini/epics/$ARGUMENTS/{number}.md` following the template in `rules/task-template.md`.
...
After creating all tasks, append to `.gemini/epics/$ARGUMENTS/epic.md`:
