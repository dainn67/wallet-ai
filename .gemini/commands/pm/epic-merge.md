...
2. **Check for active agents:**
   Read `.gemini/epics/$ARGUMENTS/execution-status.md`
...
Update `.gemini/epics/$ARGUMENTS/epic.md`:
...
feature_list=""
if [ -d ".gemini/epics/$ARGUMENTS" ]; then
  cd .gemini/epics/$ARGUMENTS
...
epic_github_line=$(grep 'github:' .gemini/epics/$ARGUMENTS/epic.md 2>/dev/null || true)
...
mkdir -p .gemini/epics/.archived/
mv .gemini/epics/$ARGUMENTS .gemini/epics/.archived/
echo "✅ Epic archived: .gemini/epics/.archived/$ARGUMENTS"
```
...
REPO=""
epic_github=$(grep '^github:' .gemini/epics/.archived/$ARGUMENTS/epic.md 2>/dev/null | head -1 | sed 's/^github: *//')
...
# Extract epic issue number
epic_github_line=$(grep 'github:' .gemini/epics/.archived/$ARGUMENTS/epic.md 2>/dev/null || true)
if [ -n "$epic_github_line" ]; then
  epic_issue=$(echo "$epic_github_line" | grep -oE '[0-9]+$' || true)
else
  epic_issue=""
...
# Close task issues
for task_file in .gemini/epics/.archived/$ARGUMENTS/[0-9]*.md; do
  [ -f "$task_file" ] || continue
...
Next steps:
  - Close epic: /pm:epic-close $ARGUMENTS
