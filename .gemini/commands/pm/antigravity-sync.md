...
Sync features from Gemini CLI to Antigravity IDE. Detects gaps and transforms command files.
...
```bash
test -f .gemini/scripts/pm/antigravity-sync.sh || { echo "❌ Script not found: .gemini/scripts/pm/antigravity-sync.sh"; exit 1; }
```
...
**If `--dry-run`:**
```bash
bash .gemini/scripts/pm/antigravity-sync.sh detect
```
Show output and stop — no files written.

**Otherwise (sync):**
```bash
bash .gemini/scripts/pm/antigravity-sync.sh sync --type "$type_filter" --yes
```
