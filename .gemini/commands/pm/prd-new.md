...
1. **Validate `$ARGUMENTS`:**
   ...
   - If invalid → print `❌ Feature name must be kebab-case (e.g. "epic-verify", "smart-context"). Got: '$ARGUMENTS'` and stop.
2. **Check existing PRD:**
   - If `.gemini/prds/$ARGUMENTS.md` exists → ask `⚠️ PRD '$ARGUMENTS' already exists. Overwrite? (yes/no)`
   - If user says no → stop.
3. **Check draft resume:**
   - If `.gemini/prds/.draft-$ARGUMENTS.md` exists → ask `📝 Found unfinished draft for '$ARGUMENTS'. Resume where you left off? (yes/no)`
   - If yes → read draft, skip completed discovery waves, continue from last checkpoint.
   - If no → delete draft and start fresh.
4. **Ensure directory:** `mkdir -p .gemini/prds 2>/dev/null`
...
- `.gemini/context/product-context.md` — Existing users and use cases
- `.gemini/context/tech-context.md` — Technical constraints and stack
- `.gemini/context/project-brief.md` — Project goals and success criteria
- `.gemini/prds/` — Scan existing PRD filenames to avoid overlap and understand feature landscape

**Codebase scan (lightweight):**
- Read `package.json` / `Cargo.toml` / equivalent if exists — note language, framework, test setup
- Check `.gemini/epics/*/epic.md` frontmatter for active epics (detect overlap)
- Scan `.gemini/prds/` executive summaries for semantic overlap with this feature
...
→ Ask these questions then STOP. When user responds, save checkpoint to `.gemini/prds/.draft-$ARGUMENTS.md` with discovery notes, then proceed to Wave 2.
...
Save the completed PRD to: `.gemini/prds/$ARGUMENTS.md`
...
1. **Clean up:** Delete draft file if exists: `rm -f .gemini/prds/.draft-$ARGUMENTS.md`
2. **Confirm:** `✅ PRD created: .gemini/prds/$ARGUMENTS.md`
3. **Summary:** Show a 3-5 line recap: problem, primary users, key risks, priority.
4. **Next steps:**
   ```
   📋 Next actions:
   → Validate PRD:   /pm:prd-validate $ARGUMENTS
   → Review & edit:  /pm:prd-edit $ARGUMENTS
