...
2. Check input assembly script exists:
   ```bash
   test -f .gemini/scripts/epic-input-assembly.sh || echo "❌ Input assembly script not found at .gemini/scripts/epic-input-assembly.sh"
   ```

3. Check config:
   ```bash
   if [ -f .gemini/config/epic-verify.json ]; then
     python3 -c "import json; c=json.load(open('.gemini/config/epic-verify.json')); print('disabled' if not c.get('phase_a',{}).get('enabled',True) else 'ok')"
   fi
   ```
   If disabled: "❌ Phase A is disabled in config. Update .gemini/config/epic-verify.json"
...
rm -rf "$OUTPUT_DIR"
bash .gemini/scripts/epic-input-assembly.sh "$EPIC_NAME" "$OUTPUT_DIR"
```
...
Read the prompt template from `.gemini/prompts/epic-phase-a.md`.

Replace placeholders with actual values:
- `{epic_name}` → the epic name from $ARGUMENTS
...
Write report to: `.gemini/context/verify/epic-reports/{epic-name}-{YYYYMMDD-HHMMSS}.md`
...
Epic: {epic-name}
Report: .gemini/context/verify/epic-reports/{filename}
...
- Input assembly fails → "❌ Input assembly failed: {error}. Check .gemini/scripts/epic-input-assembly.sh"
- No issues found → "❌ No issues found for epic '$ARGUMENTS'. Check GitHub labels."
...
- **Proceed to Phase B** → `/pm:epic-verify-b $ARGUMENTS`
- **Fix gaps first** → `/pm:epic-fix-gap $ARGUMENTS {id}` then re-run `/pm:epic-verify-a $ARGUMENTS`
- **Accept gaps** → `/pm:epic-accept-gaps $ARGUMENTS {ids}` then `/pm:epic-verify-b $ARGUMENTS`
- **Abort** → Continue development, re-run `/pm:epic-verify $ARGUMENTS` later
