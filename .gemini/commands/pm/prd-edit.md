...
2. **Locate PRD:** `.gemini/prds/$ARGUMENTS.md` must exist. If not → `❌ PRD not found. Run: /pm:prd-new $ARGUMENTS`
...
- `.gemini/prds/$ARGUMENTS.md` — the PRD
- `.gemini/prds/.validation-$ARGUMENTS.md` — validation report (if exists)
- `.gemini/context/tech-context.md` — technical constraints
- `.gemini/epics/$ARGUMENTS/epic.md` — associated epic (if exists)
...
⚠️ Downstream impact detected:
  ...
  Epic .gemini/epics/$ARGUMENTS/epic.md may need updating.
```
