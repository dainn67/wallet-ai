...
```
No active verification for epic '$ARGUMENTS'.

Last reports:
  Run: /pm:epic-verify-history $ARGUMENTS to see past runs.
Start new:
  Run: /pm:epic-verify $ARGUMENTS
```

### 2. Check Epic Match

Verify the `active_epic.epic_name` matches `$ARGUMENTS`. If not:
```
⚠️ Active verification is for epic '{active_epic_name}', not '$ARGUMENTS'.

Active: /pm:epic-verify-status {active_epic_name}
History: /pm:epic-verify-history $ARGUMENTS
```
