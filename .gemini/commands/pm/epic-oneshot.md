...
2. **Locate epic:**
   - If `.gemini/epics/$ARGUMENTS/epic.md` doesn't exist → `❌ Epic not found. Run: /pm:prd-parse $ARGUMENTS` and stop.

3. **Check existing tasks:**
   - If task files (`[0-9]*.md`) exist in `.gemini/epics/$ARGUMENTS/`:
     ```
...
Tasks are saved locally in .gemini/epics/$ARGUMENTS/
