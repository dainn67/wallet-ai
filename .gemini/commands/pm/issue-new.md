...
2. **Ensure sync script:**
   - `test -f .gemini/scripts/pm/issue-new-sync.sh || echo "❌ Sync script missing. Run: /pm:epic-run issue-new (T001 creates it)"`
3. **Ensure directories:**
   - `mkdir -p .gemini/context/sessions 2>/dev/null`
...
- `.gemini/context/system-patterns.md` — Architecture patterns in use
- `.gemini/context/project-structure.md` — Directory layout and conventions
- `.gemini/context/skillbook.md` — Known patterns and past solutions
...
4. **Call sync script:**
   ```bash
   issue_number=$(bash .gemini/scripts/pm/issue-new-sync.sh create "$title" "$body_file" "$labels_csv")
   ```
