...
init_verify_state $ARGUMENTS "$(basename $(ls -d .gemini/epics/*/  | head -1))"
```
...
```bash
template=$(bash .gemini/scripts/knowledge-extract.sh extract $ARGUMENTS 2>/dev/null || echo "")
```
...
if [ -f ".gemini/context/sessions/issue-${ARGUMENTS}-debug.md" ]; then
  summary=$(bash .gemini/scripts/debug-journal-archive.sh archive $ARGUMENTS 2>/dev/null || echo "")
