...
2. **Locate PRD:**
   - If `.gemini/prds/$ARGUMENTS.md` doesn't exist → `❌ PRD not found: .gemini/prds/$ARGUMENTS.md. Run: /pm:prd-new $ARGUMENTS` and stop.
3. **PRD quality gate:**
   ...
   - If any missing → `⚠️ PRD '$ARGUMENTS' is incomplete — missing: [list]. Run: /pm:prd-edit $ARGUMENTS to complete it first.` and stop.
   - Check requirement format:
     - If Requirements section has FR-1/NFR-1 format IDs → good, proceed
     - If no IDs → `⚠️ PRD uses legacy format without requirement IDs. Recommend: /pm:prd-edit $ARGUMENTS to add IDs. Continuing with auto-assigned IDs.`
   - Check validation status:
     - If `_Metadata.validation_status` exists and is "failed" → `⚠️ PRD failed validation. Consider: /pm:prd-validate $ARGUMENTS first.` (warn, don't block)
4. **Check existing epic:**
   - If `.gemini/epics/$ARGUMENTS/epic.md` exists → ask `⚠️ Epic '$ARGUMENTS' already exists. Overwrite? (yes/no)`
   - If no → stop.
5. **Check overlap with active epics:**
   - Scan `.gemini/epics/*/epic.md` for epics with `status: in-progress` or `status: backlog`.
...
6. **Ensure directory:** `mkdir -p .gemini/epics/$ARGUMENTS 2>/dev/null`
...
- `.gemini/context/tech-context.md` — Technical stack and constraints
- `.gemini/context/system-patterns.md` — Existing architecture patterns
- `.gemini/context/project-structure.md` — Directory and module organization
- `.gemini/context/product-context.md` — User personas and use cases
...
**Only if** `.gemini/designs/$ARGUMENTS/` directory exists:
- Read `.gemini/designs/$ARGUMENTS/design-system.md` if it exists — store as design context (color palette, typography, spacing scale, component patterns) for epic generation
- If `.gemini/designs/$ARGUMENTS/specs/` directory has files — list spec filenames for task enrichment later
...
Load `.gemini/prds/$ARGUMENTS.md` and perform deep analysis:
...
**Decision:** Use design system at `.gemini/designs/$ARGUMENTS/design-system.md` as the visual language foundation. All UI tasks reference design tokens instead of hardcoding values.
**Source:** `.gemini/designs/$ARGUMENTS/design-system.md`
...
**Only if** screen spec files were found in `.gemini/designs/$ARGUMENTS/specs/`:
- For each task that involves UI implementation (task description mentions screens, components, or UI):
  - Add to the task's "What" field: "Reference design spec: `.gemini/designs/$ARGUMENTS/specs/{screen}-spec.md`"
...
Create: `.gemini/epics/$ARGUMENTS/epic.md`
...
1. **Confirm:** `✅ Epic created: .gemini/epics/$ARGUMENTS/epic.md`
...
4. **Next steps:**
   ```
   📋 Next actions:
   → Decompose into tasks:  /pm:epic-decompose $ARGUMENTS
   → One-shot (tasks + sync): /pm:epic-oneshot $ARGUMENTS
   → View epic details:      /pm:epic-show $ARGUMENTS
   ```
