...
This command updates the project context documentation in `.gemini/context/` to reflect the current state of the project. Run this at the end of each development session to keep context accurate.
...
**File Modifications:**
- Check context file ages: `find .gemini/context -name "*.md" -type f -exec ls -lt {} + | head -5`
...
### 7. Update README.md (if context files changed)

If any context files were updated in this run:

1. Read the updated context files
2. Read existing README.md
3. Update only the sections that correspond to changed context files:
   - project-overview changed → update Features section
   - tech-context changed → update Tech Stack / Installation sections
   - project-structure changed → update Project Structure section
4. Preserve all other sections unchanged
5. Display: `📝 README.md updated to reflect context changes`

If no context files changed, skip: `⏭️ README.md — no context changes`

### 8. Incremental Update Tracking
