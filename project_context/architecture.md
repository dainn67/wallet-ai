...
- **Schema**:
  ...
  - `Category`: User-defined or default classification. Supports `parent_id` for grouping (single-level: roots have `parent_id == -1`, subs point to a root). Re-parenting goes through `RecordRepository.updateCategoryAndReparent`, which transactionally moves any existing children of the edited category to share the new parent so the hierarchy stays flat. A seeded `'Transfer'` category (type `'transfer'`) is used for transfer rows. Carries a non-null `emoji` field (default `'🏷️'`); `Category.fromMap` coalesces null/empty DB values to `'🏷️'`. Schema v9 → v10 migration (`RecordMigrationService.addEmojiColumn`) adds the column via `ALTER TABLE ADD COLUMN` and seeds 17 curated emojis. The emoji renders in three places: the leading `Text` in `CategoryWidget` (CategoriesTab rows), the 36×36 circular leading badge in `RecordWidget` (replacing the prior direction glyph; subtitle now omits the emoji prefix), and inline in `SuggestionBanner.suggestion.message` (server-supplied).
