...
Tapping a money source on the Records tab opens `EditSourcePopup`, which exposes a leading transfer icon (`Icons.swap_horiz`). Tapping it closes the popup and opens `TransferPopup` with the current source locked as "From"; the user picks a destination, an amount, and an optional note. A transfer is persisted as a single `Record` row with `type = 'transfer'`, `money_source_id` = origin, and a new `target_source_id` column = destination. `RecordRepository._applyRecordImpact` debits the origin and credits the destination atomically (mirrored on delete). Transfers are excluded from `filteredTotalIncome`/`filteredTotalExpense` and the home-widget monthly totals, but appear in the records list rendered with an indigo `swap_horiz` glyph and a "From → To" subtitle. v1 is delete-only — opening a transfer in `EditRecordPopup` shows a read-only summary with a Delete button. Schema migration v8 → v9 rebuilds the `Record` table (copy → drop → rename) to add the nullable `target_source_id` column and relax the `type` CHECK constraint. See `docs/features/transfer.md`.
...
### Inactivity Reminders (Local Notifications)
Drawer entry **Reminders** (drawer `SwitchListTile` in `home_screen.dart`) toggles a SharedPreferences flag `reminders_enabled`. On first launch (`main.dart::_maybeAskNotificationPermissionOnce`), the OS permission prompt is shown once; the toggle defaults to the user's answer. After that, the user can flip it from the drawer — turning it ON while permission is revoked surfaces a `ConfirmationDialog` that routes to system settings (`NotificationService.openSystemSettings`).

The reminder ladder is declared in `lib/configs/notification_config.dart` as a `List<ReminderConfig>` (id, daysAfterLastRecord, titleKey, bodyKey). Default ladder: days 1 / 3 / 5 / 7, all firing at 20:00 local time using `AndroidScheduleMode.inexactAllowWhileIdle` (no `SCHEDULE_EXACT_ALARM` permission required). Copy lives in `L10nConfig` under the `notif_dayN_title` / `notif_dayN_body` keys (EN + VI), in Wally's first-person voice.

`NotificationProvider` (`ChangeNotifierProxyProvider2<RecordProvider, LocaleProvider, NotificationProvider>`) is the single coordinator: its `attach` callback fires whenever records or locale change, re-applying the schedule via `NotificationService.scheduleInactivityReminders(lastRecordAt, translate)`. The latest `Record.occurredAt` is used as the anchor — past fire times are silently skipped. Toggling OFF cancels all pending reminders via `NotificationService.cancelAllReminders`.

### Re-parent Category
`CategoryFormDialog` (lib/components/popups/category_form_dialog.dart) shows a "Parent Category" dropdown for every non-Uncategorized row. Picking *None* keeps/promotes the category to top-level; picking a root demotes it into a sub. When the edited category currently has sub-categories AND it is being demoted to a sub itself, all of its existing children cascade under the new parent (kept single-level — there are no grand-sub-categories). Eligible parents in the dropdown are filtered to roots (`parent_id == -1`) of the same type, excluding self and Uncategorized. Persistence lives in `RecordRepository.updateCategoryAndReparent({category, oldParentId})` which performs both the cascade `UPDATE Category SET parent_id = ? WHERE parent_id = <editedId>` and the main row update in a single SQLite transaction. The provider exposes it as `RecordProvider.updateCategoryAndReparent(category, oldParentId)`.
