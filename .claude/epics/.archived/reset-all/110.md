...
## Acceptance Criteria
- [x] **FR-2 / Test:** `resetAllData` is tested to clear `Record` table and zero `MoneySource` amounts.
- [x] **FR-3 / Test:** `deleteMoneySource` is tested to delete the source and its related `Record` entries.
- [x] **NFR-1 / Test:** State updates in `RecordProvider` are verified after these actions.
