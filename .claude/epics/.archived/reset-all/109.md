...
## Acceptance Criteria
- [x] **FR-3 / UI:** A distinct "Delete" button (icon or text) is visible in the edit popup.
- [x] **FR-3 / Warning:** Clicking delete shows the `ConfirmationDialog` explicitly warning about associated records.
- [x] **FR-3 / Execution:** Confirming calls `context.read<RecordProvider>().deleteMoneySource(sourceId)` and closes both popups.
