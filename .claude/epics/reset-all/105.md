...
## Acceptance Criteria
- [x] **FR-1 / Happy path:** Calling the widget displays a dialog with the provided title, content, and buttons.
- [x] **FR-1 / Action:** Clicking "Confirm" executes the `onConfirm` callback and closes the dialog.
- [x] **FR-1 / Cancel:** Clicking "Cancel" simply closes the dialog without executing `onConfirm`.
- [x] **NFR-3 / Styling:** Dialog uses the `Poppins` font. If `isDestructive` is true, the confirm button has a red text/background color to indicate danger.
