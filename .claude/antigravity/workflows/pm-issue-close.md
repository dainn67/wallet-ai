...
If not found, search for task file with `github:.*issues/$ISSUE_NUMBER` in frontmatter (old naming).
...
If progress file exists at `.claude/epics/{epic}/updates/$ISSUE_NUMBER/progress.md`:
- Set completion: 100%
...
---
Closed at: {timestamp}" | gh issue comment $ISSUE_NUMBER --repo "$REPO" --body-file -
...
epic_name=$(ls .claude/epics/*/[0-9]*.md 2>/dev/null | grep "/${ARGUMENTS}.md$" | head -1 | sed 's|.claude/epics/||' | cut -d'/' -f1)

if [ -n "$epic_name" ] && [ -f ".claude/epics/$epic_name/epic.md" ]; then
  # Count tasks
  total=0
  closed=0
  ...
  if [ $total -gt 0 ]; then
    ...
    sed -i.bak "s/^progress: .*/progress: ${percent}%/" ".claude/epics/$epic_name/epic.md" && rm -f ".claude/epics/$epic_name/epic.md.bak"
    sed -i.bak "s/^updated: .*/updated: ${updated_at}/" ".claude/epics/$epic_name/epic.md" && rm -f ".claude/epics/$epic_name/epic.md.bak"
...
Follow `.claude/rules/frontmatter.md` for updates.
