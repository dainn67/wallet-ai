...
2. **Get issue details:**
   ...
   **If IS_LIGHT_PATH=true — Create debug journal:**
   ```bash
   mkdir -p .claude/context/sessions/archive 2>/dev/null
   ```
   Ask user to choose journal mode:
   ```
   Debug journal mode cho issue #$ISSUE_NUMBER:
   ...
   ```
   Then create `.claude/context/sessions/issue-$ISSUE_NUMBER-debug.md`:
   ```markdown
   # Debug Journal: Issue #$ISSUE_NUMBER — {title}
...
3. **Find local task file:**
   ...
   - First check if `.claude/epics/*/$ISSUE_NUMBER.md` exists (new naming)
   - If not found, search for file containing `github:.*issues/$ISSUE_NUMBER` in frontmatter (old naming)
   ...
   **Detect branch strategy (after task file check):**
   ...
   elif [[ "$ISSUE_NUMBER" == *"--branch"* ]]; then
     BRANCH_STRATEGY="branch"   # User override: force branch
   elif [[ "$ISSUE_NUMBER" == *"--no-branch"* ]]; then
...
6. **Check for analysis:**
   ...
   ```bash
   test -f .claude/epics/*/$ISSUE_NUMBER-analysis.md || echo "❌ No analysis found for issue #$ISSUE_NUMBER
...
> **SCOPE: This command processes EXACTLY ONE issue (#$ISSUE_NUMBER). After setup and output, STOP. Never continue to additional issues, never call issue-complete, never look for next tasks.**
...
if [ "$BRANCH_STRATEGY" = "branch" ]; then
  ...
  echo "Created branch: fix/issue-$ISSUE_NUMBER"
else
...
1. **Load previous context**: If `.claude/context/handoffs/latest.md` exists, read it and summarize key points: "I understand that..."
2. **Read epic context**: Check for epic-level context files in `.claude/context/epics/`
3. **Skillbook Injection (ace-learning)**: If ace-learning skillbook is enabled, inject relevant learned patterns:
   ```bash
   source .claude/scripts/pm/skillbook-inject.sh 2>/dev/null || true
...
```bash
test -f .claude/hooks/pre-task.sh && bash .claude/hooks/pre-task.sh
```
...
Task:
  ...
  prompt: |
    You are working on Issue #$ISSUE_NUMBER in the epic worktree.
    ...
    4. Update progress in: .claude/epics/{epic_name}/updates/$ISSUE_NUMBER/stream-{X}.md
    5. Follow coordination rules in .claude/rules-reference/agent-coordination.md
...
gh issue edit $ISSUE_NUMBER --repo "$REPO" --add-assignee @me --add-label "in-progress"
```
...
**If IS_LIGHT_PATH=true:**
```
✅ Started work on standalone issue #$ISSUE_NUMBER

Branch: {current_branch} (direct commit) | fix/issue-$ISSUE_NUMBER (new branch)
Debug journal: .claude/context/sessions/issue-$ISSUE_NUMBER-debug.md (mode: {mode})

Workflow:
  1. During work:    /pm:verify-run
  2. Complete issue:  /pm:issue-complete $ISSUE_NUMBER
```
...
Workflow:
  ...
  2. Complete issue:  /pm:issue-complete $ISSUE_NUMBER (handoff + verify + close)
