...
init_verify_state $ISSUE_NUMBER "$(basename $(ls -d .claude/epics/*/  | head -1))"
```
...
```bash
template=$(bash .claude/scripts/knowledge-extract.sh extract $ISSUE_NUMBER 2>/dev/null || echo "")
```
...
if [ -f ".claude/context/sessions/issue-${ARGUMENTS}-debug.md" ]; then
  summary=$(bash .claude/scripts/debug-journal-archive.sh archive $ISSUE_NUMBER 2>/dev/null || echo "")
...
3. If healthy, generate a structured task summary by analyzing:
   - The task file at `.claude/epics/*/$(printf '%03d' $ISSUE_NUMBER).md` or similar (if available)
