...
[ -z "$EPIC_NAME" ] && { echo "❌ Usage: /pm:epic-run <epic_name> [flags]"; exit 1; }

# 2. Extract epic name (first argument) and flags
epic_name=$(echo "$EPIC_NAME" | awk '{print $1}')

# 3. Epic must exist
test -f .claude/epics/$epic_name/epic.md || { echo "❌ Epic not found. Run: /pm:prd-parse $epic_name"; exit 1; }
...
Parse flags from `$EPIC_NAME` after the epic name:
...
# Parse flags
shift_args=$(echo "$EPIC_NAME" | cut -d' ' -f2-)
...
**Use `$epic_name` instead of `$EPIC_NAME`** for all epic references below.
...
```bash
bash .claude/scripts/pm/epic-run-plan.sh $epic_name
```
...
if [ -f .claude/config/epic-run.json ]; then
  max_parallel=$(grep -o '"max_parallel": *[0-9]*' .claude/config/epic-run.json | grep -o '[0-9]*')
  graceful_degradation=$(grep -o '"graceful_degradation": *[a-z]*' .claude/config/epic-run.json | grep -o '[a-z]*$')
  model_override=$(grep -o '"model_override": *"[^"]*"' .claude/config/epic-run.json | grep -o '"[^"]*"$' | tr -d '"')
  skip_verification=$(grep -o '"skip_verification": *[a-z]*' .claude/config/epic-run.json | grep -o '[a-z]*$')
fi
...
completed_count=0
if [ -f .claude/config/epic-run.json ]; then
  seq_sub=$(grep -o '"sequential_subagent": *[a-z]*' .claude/config/epic-run.json | grep -o '[a-z]*$')
  max_len=$(grep -o '"max_result_length": *[0-9]*' .claude/config/epic-run.json | grep -o '[0-9]*')
  warn_n=$(grep -o '"warn_after_tasks": *[0-9]*' .claude/config/epic-run.json | grep -o '[0-9]*')
...
4. **File conflict check:** For each parallel candidate group:
   - Read each task file (`.claude/epics/{epic}/{num}.md`) and extract `files:` and `conflicts_with:` frontmatter arrays
...
Task:
  ...
  prompt: |
    ...
    ```bash
    cat .claude/epics/{epic_name}/{task_num}.md
    ```

    ## Context
    Read handoff from previous task:
    ```bash
    cat .claude/context/handoffs/latest.md 2>/dev/null || echo "No previous handoff"
    ```

    Read epic context:
    ```bash
    cat .claude/context/epics/{epic_name}.md 2>/dev/null || echo "No epic context"
    ```
    ...
    5. Write handoff notes to .claude/context/handoffs/latest.md
    ...
    6. Update task file status — set `status: closed` and `updated` in frontmatter:
       ```bash
       sed -i '' 's/^status: .*/status: closed/' .claude/epics/{epic_name}/{task_num}.md
       sed -i '' "s/^updated: .*/updated: $(date -u +"%Y-%m-%dT%H:%M:%SZ")/" .claude/epics/{epic_name}/{task_num}.md
       ```
...
Get the GitHub repo:
```bash
REPO=$(bash .claude/scripts/pm/github-helpers.sh get-repo 2>/dev/null || echo "")
```
...
**Critical:** All Task tool calls for the group MUST be in the **same message** so Claude Code executes them concurrently.
