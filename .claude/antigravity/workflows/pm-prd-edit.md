...
2. **Locate PRD:** `.claude/prds/$FEATURE_NAME.md` must exist. If not → `❌ PRD not found. Run: /pm:prd-new $FEATURE_NAME`
...
- `.claude/prds/$FEATURE_NAME.md` — the PRD
...
⚠️ Downstream impact detected:
  ...
  Epic .claude/epics/$FEATURE_NAME/epic.md may need updating.
```
