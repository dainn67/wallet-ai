...
task_count=$(ls .claude/epics/$EPIC_NAME/[0-9]*.md 2>/dev/null | grep -cv epic.md)
[ "$task_count" -eq 0 ] && { echo "❌ No tasks to sync. Run: /pm:epic-decompose $EPIC_NAME"; exit 1; }

# 3. Check if already synced (idempotency)
if [ -f ".claude/epics/$EPIC_NAME/github-mapping.md" ]; then
  echo "⚠️ Epic already synced to GitHub. Mapping file exists."
  echo "To re-sync, first delete: .claude/epics/$EPIC_NAME/github-mapping.md"
  exit 0
fi
...
sed '1,/^---$/d; 1,/^---$/d' .claude/epics/$EPIC_NAME/epic.md > /tmp/epic-body.md
...
for task_file in .claude/epics/$EPIC_NAME/[0-9]*.md; do
  ...
  if [ "$use_subissues" = true ]; then
    task_url=$(gh sub-issue create \
      ...
      --label "task,epic:$EPIC_NAME,model:$recommended_model")
  else
...
Read `.claude/epics/$EPIC_NAME/epic.md` with the Read tool. Then use the Write tool to update:
...
✅ Synced to GitHub
  ...
  - Mapping: .claude/epics/$EPIC_NAME/github-mapping.md

Next steps:
  - Create branch: /pm:epic-start $EPIC_NAME
