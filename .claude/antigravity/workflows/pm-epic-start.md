...
2. **Read epic context**: If `.claude/context/epics/$EPIC_NAME.md` exists, read for epic-level decisions and history
3. **Review epic details**: Re-read `.claude/epics/$EPIC_NAME/epic.md` for acceptance criteria and overall scope
...
If `.claude/context/epics/$EPIC_NAME.md` does not exist, create it with:
```markdown
---
epic: $EPIC_NAME
...
Read all task files in `.claude/epics/$EPIC_NAME/` (files matching `[0-9]*.md`).
...
3. **Create/update gap fix plan file** at `.claude/context/epics/$EPIC_NAME-gap-fixes.md`:
...
Create `.claude/epics/$EPIC_NAME/execution-status.md` with current state snapshot:
- Branch name
