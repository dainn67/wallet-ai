---
name: pm-epic-verify-resume
description: Epic Verify — Resume
# tier: heavy
---
...
**If `active_epic` is non-null and matches `$EPIC_NAME`:**
- Verification is still active (possibly from a crashed session)
- Continue from current state

**If `active_epic` is null but `last_abort` exists and matches `$EPIC_NAME`:**
- Verification was explicitly aborted
- Can resume from abort point

**If neither:**
```
❌ No verification state found for epic '$EPIC_NAME'.
...
```bash
bash .claude/scripts/pm/lifecycle-helpers.sh init-epic-verify-state "$EPIC_NAME" "{phase_a_report_from_abort_or_latest}"
```
...
Read and execute Phase B instructions from `.claude/commands/pm/epic-verify-b.md`, but skip steps that have already been completed:
...
- Phase A report missing → "❌ Phase A report not found. Run /pm:epic-verify $EPIC_NAME to start fresh."
