...
1. **Find local task file:**
   - First check if `.claude/epics/*/$ISSUE_NUMBER.md` exists (new naming convention)
...
2. **Check for existing analysis:**
   ```bash
   test -f .claude/epics/*/$ISSUE_NUMBER-analysis.md && echo "⚠️ Analysis already exists. Overwrite? (yes/no)"
   ```
...
Create `.claude/epics/{epic_name}/$ISSUE_NUMBER-analysis.md`:
