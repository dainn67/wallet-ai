...
2. **Locate epic:**
   - If `.claude/epics/$EPIC_NAME/epic.md` doesn't exist → `❌ Epic not found: .claude/epics/$EPIC_NAME/epic.md. Run: /pm:prd-parse $EPIC_NAME` and stop.
3. **Epic quality gate:**
...
4. **Check existing tasks:**
   - Scan `.claude/epics/$EPIC_NAME/` for task files (`[0-9]*.md`).
...
Check if `.claude/designs/$EPIC_NAME/specs/` directory exists and contains `.md` files:
- If yes: build a map of screen-name to spec-file-path (e.g., `dashboard` → `.claude/designs/$EPIC_NAME/specs/dashboard-spec.md`). This map will be used in Step 5 to enrich UI-related tasks.
...
Load `.claude/epics/$EPIC_NAME/epic.md` and extract:
...
For each task, create `.claude/epics/$EPIC_NAME/{number}.md` following the template in `.claude/rules-reference/task-template.md`.
...
After creating all tasks, append to `.claude/epics/$EPIC_NAME/epic.md`:
