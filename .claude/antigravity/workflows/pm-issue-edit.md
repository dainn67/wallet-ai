...
# Search for file with github:.*issues/$ISSUE_NUMBER
```
...
If body changed:
```bash
gh issue edit $ISSUE_NUMBER --repo "$REPO" --body-file {updated_task_file}
```

If labels changed:
```bash
gh issue edit $ISSUE_NUMBER --repo "$REPO" --add-label "{new_labels}"
gh issue edit $ISSUE_NUMBER --repo "$REPO" --remove-label "{removed_labels}"
```
...
Follow `.claude/rules/frontmatter.md`.
