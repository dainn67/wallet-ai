...
description: Use when switching IDE, syncing context, coming from Claude Code, switching to Antigravity, or switching to Claude Code. NOT for: general task start, writing handoff notes, or running verification.
---

# CCPM Context Sync

Detect and handle an IDE switch — load transition context when moving between Claude Code and Antigravity.
...
3. Load `.claude/context/handoffs/latest.md` and present the transition context
4. Confirm the active state to the user before proceeding with any work

## When This Applies

- You are in Antigravity but the last session was in Claude Code (or another IDE)
- The user explicitly says "I was just in Claude Code" or "switching IDEs"
- The `active-ide.json` file shows a different `last_ide`

## Output Format

```
🔄 IDE Switch detected: claude-code → antigravity
