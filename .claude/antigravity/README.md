...
Adapter layer for running CCPM (Claude Code Project Manager) inside Google Antigravity IDE.
...
**Key principle:** `.agent/` contains only Antigravity adapter files. All CCPM state lives in `.claude/`.

## Workflows (28)

Workflows are user-triggered via `/command` in Antigravity. Each maps to a Claude Code `/pm:*` command.
...
| `ccpm-commit-standards.md` | Branch naming, commit format, `.claude/` exclusion |
| `ccpm-enforcement.md` | Self-enforcement protocol with checklists and anti-bypass patterns |
| `ccpm-path-convention.md` | `.agent/` is adapter-only, all state in `.claude/` |

## Enforcement Model

### The Gap

Claude Code enforces CCPM rules with **hard hooks** — `pre-tool-use.sh` can BLOCK tool calls (`exit 2`). Antigravity has NO hook system. Rules are advisory only.
...
5. **Pair Antigravity with Claude Code** — use Claude Code for critical tasks where hard enforcement matters, Antigravity for exploratory and visual work
