...
# --- Test 8: .claude/settings.local.json takes priority ---
echo ""
echo "Test 8: .claude/settings.local.json priority"
TMPDIR_TEST=$(mktemp -d)
mkdir -p "$TMPDIR_TEST/.claude"
cat > "$TMPDIR_TEST/.claude/settings.local.json" << 'SETTINGSEOF'
{
  "mcpServers": {
    "stitch-design": {
      "command": "stitch"
...
output=$(cd "$TMPDIR_TEST" && bash "$SCRIPT" 2>&1)
actual_exit=$(cd "$TMPDIR_TEST" && bash "$SCRIPT" >/dev/null 2>&1; echo $?)
assert_ok "exits 0 for .claude/settings.local.json with stitch" "$([ "$actual_exit" = "0" ] && echo true || echo false)"
assert_ok "stdout contains 'stitch-mcp:available'" "$(echo "$output" | grep -q 'stitch-mcp:available' && echo true || echo false)"
rm -rf "$TMPDIR_TEST"
