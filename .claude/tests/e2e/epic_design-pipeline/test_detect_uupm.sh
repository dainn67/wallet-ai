...
TMPDIR_TEST=$(mktemp -d)
mkdir -p "$TMPDIR_TEST/.claude-plugin/skills/ui-ux-pro-max"
echo "# UI UX Pro Max Skill" > "$TMPDIR_TEST/.claude-plugin/skills/ui-ux-pro-max/SKILL.md"
...
TMPDIR_TEST=$(mktemp -d)
mkdir -p "$TMPDIR_TEST/.claude-plugin/skills/ui-ux-pro-max"
echo "# search module" > "$TMPDIR_TEST/.claude-plugin/skills/ui-ux-pro-max/search.py"
output=$(cd "$TMPDIR_TEST" && bash "$SCRIPT" 2>&1)
actual_exit=$(cd "$TMPDIR_TEST" && bash "$SCRIPT" >/dev/null 2>&1; echo $?)
