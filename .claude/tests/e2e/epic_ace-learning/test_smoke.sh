...
# Use .claude/ prefix for runtime paths
...
assert_file ".claude/config/ace-learning.json" "Config file exists"
python3 -c "import json; json.load(open('.claude/config/ace-learning.json'))" 2>/dev/null
assert_exit 0 $? "Valid JSON"

run_test "Config has required sections"
config=$(cat .claude/config/ace-learning.json)
...
output=$(grep -c "read_ace_config\|ace_feature_enabled\|ace_log" .claude/scripts/pm/lifecycle-helpers.sh 2>/dev/null)
...
assert_file ".claude/scripts/pm/skillbook-extract.sh" "Script exists"
...
assert_file ".claude/commands/pm/skillbook.md" "View command"

run_test "skillbook-prune.md command exists"
assert_file ".claude/commands/pm/skillbook-prune.md" "Prune command"

run_test "issue-complete.md has Learning Extraction section"
assert_contains "$(cat .claude/commands/pm/issue-complete.md)" "Learning Extraction" "Extraction step present"

run_test "issue-start.md has Skillbook Injection section"
assert_contains "$(cat .claude/commands/pm/issue-start.md)" "Skillbook Injection\|skillbook-inject\|skillbook" "Injection step present"

run_test "issue-start.md has Complexity Assessment section"
assert_contains "$(cat .claude/commands/pm/issue-start.md)" "Complexity Assessment\|complexity-score\|complexity" "Scoring step present"

run_test "verify-run.md has Reflection & Retry section"
assert_contains "$(cat .claude/commands/pm/verify-run.md)" "Reflection.*Retry\|reflection-generate\|ace_feature_enabled" "Retry flow present"

# ── 4. Context files exist ──

run_test "skillbook.md context file exists"
assert_file ".claude/context/skillbook.md" "Skillbook file"

run_test "ace-learning-log.md exists"
assert_file ".claude/context/ace-learning-log.md" "Log file"
...
val=$(source .claude/scripts/pm/lifecycle-helpers.sh 2>/dev/null && read_ace_config "skillbook" "max_entries" "50" 2>/dev/null)
...
cp .claude/config/ace-learning.json /tmp/ace-learning-backup-$$.json
mv .claude/config/ace-learning.json .claude/config/ace-learning.json.bak
source .claude/scripts/pm/lifecycle-helpers.sh 2>/dev/null
ace_feature_enabled "skillbook" 2>/dev/null
rc=$?
mv .claude/config/ace-learning.json.bak .claude/config/ace-learning.json
...
deps=$(grep -rn "pip install\|npm install\|cargo install\|gem install" \
  .claude/scripts/pm/skillbook-extract.sh \
  .claude/scripts/pm/skillbook-inject.sh \
  .claude/scripts/pm/reflection-generate.sh \
  .claude/scripts/pm/complexity-score.sh \
  .claude/scripts/pm/failure-patterns.sh 2>/dev/null || true)
