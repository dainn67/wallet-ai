...
# reference source code at project root, not installed .claude/ copies.
...
if [ -f "$HOME/.claude/settings.json" ] && grep -q '"superpowers' "$HOME/.claude/settings.json" 2>/dev/null; then
...
echo '{"name":"superpowers-plugin"}' > .claude-plugin/plugin.json
bash "$PROJECT_ROOT/scripts/detect-superpowers.sh" >/dev/null 2>&1
...
output=$(echo '{"tool_name":"Write","tool_input":{"file_path":".claude/epics/integ-test/designs/task-999-design.md","content":"d"}}' | bash "$PROJECT_ROOT/hooks/pre-tool-use.sh" "$CCPM_ROOT" 2>&1)
exit_code=$?
assert "pre-tool-use: ALLOW writing design file" "0" "$exit_code"

# Create design file, then Write → ALLOW
mkdir -p ".claude/epics/integ-test/designs"
echo "# Design" > ".claude/epics/integ-test/designs/task-999-design.md"
output=$(echo '{"tool_name":"Write","tool_input":{"file_path":"/test.sh","content":"x"}}' | bash "$PROJECT_ROOT/hooks/pre-tool-use.sh" "$CCPM_ROOT" 2>&1)
exit_code=$?
