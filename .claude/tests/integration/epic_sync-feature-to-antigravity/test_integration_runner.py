...
def test_naming_convention():
    """All workflow files follow pm-{name}.md or context-{name}.md naming convention."""
    wf_dir = os.path.join(PROJECT_ROOT, 'antigravity', 'workflows')
    valid_prefixes = ('pm-', 'context-')
    for f in os.listdir(wf_dir):
        if f.endswith('.md'):
            assert f.startswith(valid_prefixes), \
                f"Workflow file {f} doesn't follow pm-{{name}}.md or context-{{name}}.md naming"
