...
1. **Test Execution**: First check if `.claude/scripts/test-and-log.sh` exists. If it does, use it for log capture. If not, read `.claude/testing-config.md` for the test command, or fall back to the project's native test command (see Error Recovery below).
...
2. **Test Execution**:
   ...
   ```bash
   # Option A: Use test-and-log script if available
   if [ -x ".claude/scripts/test-and-log.sh" ]; then
     .claude/scripts/test-and-log.sh tests/[test_file]
   # Option B: Use testing-config.md command
   elif [ -f ".claude/testing-config.md" ]; then
     # Read test_command from config frontmatter and run it
     test_cmd=$(grep '^test_command:' .claude/testing-config.md | sed 's/^test_command: *//')
     $test_cmd
   # Option C: Fall back to common framework commands (see Error Recovery)
   else
     echo "No test config found. Run /testing:prime first, or specify test command."
   fi
   ```
