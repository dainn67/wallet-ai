...
## Memory Agent Smart Loading (if available)

Before running the standard loading sequence, check if Memory Agent smart loading is available:

1. **Check config** — both `enabled` and `query_on_prime` must be true:
   ```bash
   bash -c 'source .claude/scripts/pm/lifecycle-helpers.sh && read_config_bool "memory_agent" "enabled" && read_config_bool "memory_agent" "query_on_prime"'
   ```
   If either returns non-zero (false/disabled) → skip to standard loading sequence below.

2. **Check health** — Memory Agent must be reachable:
   ```bash
   bash .claude/scripts/pm/memory-health.sh
   ```
   If exit code is non-zero → skip to standard loading sequence below.

3. **Extract task title for targeted query:**
   - Read `.claude/context/handoffs/latest.md` (if it exists)
   - Look for a line starting with `# Task:` — extract everything after `# Task:` as the task title
   - If no `# Task:` header found or file doesn't exist → use general query: `"project context recent decisions patterns"`

4. **Query Memory Agent:**
   ```bash
   source .claude/scripts/pm/lifecycle-helpers.sh
   memory_query "<task_title_or_general_query>" "markdown" "10"
   ```

5. **Evaluate results:**
   - If query returns **non-empty result**:
     - Output as `## Memory Context` section (if task title found) OR `## Memory Context (general)` (if using general query)
     - Append footer line: `---`
     - Append: `Memory: {count} memories returned (relevance-scored, limit=10)`
     - **Skip the standard loading sequence entirely** — Memory Context replaces load-all
   - If query returns **empty or fails** → proceed with standard loading sequence (unchanged behavior)

> **Note:** If Memory Agent smart loading succeeded above (output contains "## Memory Context"),
> skip the entire loading sequence below — context is already loaded via Memory Agent.
> Only run the loading sequence if Memory Agent was unavailable, disabled, or returned empty results.
...
🧠 Memory Agent: {status} ({N} memories loaded | fallback: load-all)
