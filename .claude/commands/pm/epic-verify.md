...
2. Check Phase A command exists:
   ```bash
   test -f .claude/commands/pm/epic-verify-a.md || echo "❌ Phase A command not found"
   ```

3. Check Phase B command exists:
   ```bash
   test -f .claude/commands/pm/epic-verify-b.md || echo "❌ Phase B command not found"
   ```

4. Check config:
   ```bash
   if [ -f .claude/config/epic-verify.json ]; then
     python3 -c "import json; c=json.load(open('.claude/config/epic-verify.json')); print('ok')"
   fi
   ```
...
5. Create required directories:
   ...
   mkdir -p .claude/context/epics/.archive 2>/dev/null
   ```
...
Execute Phase A by following the instructions in `.claude/commands/pm/epic-verify-a.md`, using `$ARGUMENTS` as the epic name.
...
3. Write report to `.claude/context/verify/epic-reports/{epic}-{timestamp}.md`
...
- If **option 4** → Stop here. Display: "Verification aborted. Resume with: /pm:epic-verify $ARGUMENTS"

### Phase B: Integration Verification

Execute Phase B by following the instructions in `.claude/commands/pm/epic-verify-b.md`, using `$ARGUMENTS` as the epic name.
...
```bash
EPIC_NAME="$ARGUMENTS"
PHASE_A_REPORT=$(ls -t .claude/context/verify/epic-reports/${EPIC_NAME}-*.md 2>/dev/null | grep -v "final" | head -1)
```
...
```bash
cat .claude/context/verify/epic-state.json
```
...
```bash
git log --oneline --name-only --since="$(python3 -c "import json; d=json.load(open('.claude/context/verify/epic-state.json')); print(d.get('started',''))" 2>/dev/null)" -- . 2>/dev/null | head -50
```
...
Write report to `.claude/context/verify/epic-reports/{epic}-final-{timestamp}.md`:
...
Quality Score: {X}/5
Iterations: {N}
Report: .claude/context/verify/epic-reports/{epic}-final-{timestamp}.md
```
...
if [ -f .claude/config/epic-verify.json ]; then
  eval $(python3 -c "
import json
c=json.load(open('.claude/config/epic-verify.json')).get('phase_c',{})
...
```bash
mkdir -p .claude/context/epics/.archive
if [ -f .claude/context/epics/${EPIC_NAME}.md ]; then
  mv .claude/context/epics/${EPIC_NAME}.md .claude/context/epics/.archive/
fi
```

**Commit Report:**
```bash
git add .claude/context/verify/epic-reports/
git commit -m "[Epic-Complete] ${EPIC_NAME} — verified and closed"
```

**Close GitHub Issue** (if `auto_close_epic_on_complete` is True):

First, check remote origin per `.claude/rules/github-operations.md`:
```bash
...
import json
with open('.claude/epics/${EPIC_NAME}/epic.json') as f:
    d = json.load(f)
...
" 2>/dev/null)
  ...
  REPO=""
  epic_github=$(grep '^github:' .claude/epics/${EPIC_NAME}/epic.md 2>/dev/null | head -1 | sed 's/^github: *//')
  ...
  if [ -n "$EPIC_ISSUE" ]; then
    gh issue close "$EPIC_ISSUE" --repo "$REPO" -c "Epic verified. See final report at .claude/context/verify/epic-reports/"
  fi
fi
```
...
```bash
mkdir -p .claude/context/verify/baselines
if [ -f .claude/context/verify/results/latest.json ]; then
  cp .claude/context/verify/results/latest.json .claude/context/verify/baselines/${EPIC_NAME}-baseline.json
fi
```
