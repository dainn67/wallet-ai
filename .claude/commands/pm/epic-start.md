...
grep -q '^github:' .claude/epics/$ARGUMENTS/epic.md || { echo "❌ Epic not synced. Run: /pm:epic-sync $ARGUMENTS"; exit 1; }

# 3. Auto-commit uncommitted changes
if [ -n "$(git status --porcelain)" ]; then
  echo "📦 Uncommitted changes detected. Auto-committing..."
  git add -A
  git commit -m "WIP: auto-commit before epic-start $ARGUMENTS"
...
2. **Read epic context**: If `.claude/context/epics/$ARGUMENTS.md` exists, read for epic-level decisions and history
3. **Review epic details**: Re-read `.claude/epics/$ARGUMENTS/epic.md` for acceptance criteria and overall scope
...
if [ ! -f .claude/context/epics/$ARGUMENTS.md ]; then
  mkdir -p .claude/context/epics
fi
```

If `.claude/context/epics/$ARGUMENTS.md` does not exist, create it with:
```markdown
---
epic: $ARGUMENTS
...
Read all task files in `.claude/epics/$ARGUMENTS/` (files matching `[0-9]*.md`).
...
1. If `.claude/context/verify/epic-state.json` exists and contains state for this epic:
...
2. If `.claude/context/verify/epic-reports/` contains reports matching this epic name:
...
if test -f .claude/scripts/detect-superpowers.sh; then
  bash .claude/scripts/detect-superpowers.sh && SUPERPOWERS_INSTALLED=true
fi
```
...
3. **Create/update gap fix plan file** at `.claude/context/epics/$ARGUMENTS-gap-fixes.md`:
...
Create `.claude/epics/$ARGUMENTS/execution-status.md` with current state snapshot:
- Branch name
