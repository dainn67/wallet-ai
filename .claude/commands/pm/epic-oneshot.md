...
2. **Locate epic:**
   - If `.claude/epics/$ARGUMENTS/epic.md` doesn't exist → `❌ Epic not found. Run: /pm:prd-parse $ARGUMENTS` and stop.

3. **Check existing tasks:**
   - If task files (`[0-9]*.md`) exist in `.claude/epics/$ARGUMENTS/`:
     ```
     ⚠️ Found {count} existing tasks for '$ARGUMENTS'.
     Options:
       → Delete and recreate: answer 'recreate'
       → Sync existing tasks: run /pm:epic-sync $ARGUMENTS
       → Abort: answer 'abort'
     ```
...
4. **Check if already synced:**
   - Read epic frontmatter. If `github` field has a non-empty value (not `""`, not placeholder) → epic already synced:
     ```
     ⚠️ Epic '$ARGUMENTS' already synced to GitHub (#{issue_number}).
     Use /pm:epic-sync $ARGUMENTS to update instead.
     ```
     Stop.
...
Execute: `/pm:epic-decompose $ARGUMENTS`

**On success:** Continue to Step 2.
**On failure:** Stop and report:
```
❌ Decomposition failed for '$ARGUMENTS'.
Fix the issue above, then retry: /pm:epic-oneshot $ARGUMENTS
```
...
Tasks are saved locally in .claude/epics/$ARGUMENTS/
To retry sync only: /pm:epic-sync $ARGUMENTS
To start over:      delete task files and run /pm:epic-oneshot $ARGUMENTS
```
...
📋 Next actions:
  → Start all parallel tasks:  /pm:epic-start $ARGUMENTS
  → Start single task:         /pm:issue-start {first_task_number}
  → View epic dashboard:       /pm:epic-show $ARGUMENTS
```
