...
epic_name=$(basename $(dirname $(ls .claude/epics/*/$ARGUMENTS.md 2>/dev/null | head -1)))

init_verify_state $ARGUMENTS "$epic_name"
```
...
When you're done:
  ...
  2. Complete task: /pm:issue-complete $ARGUMENTS
...
- The stop hook (`.claude/hooks/stop-verify.sh`) will run automatically when you try to exit
