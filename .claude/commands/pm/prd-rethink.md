...
description: |
  CEO/Founder-mode product rethink. Challenge premises, find the 10-star product
  hiding inside a feature request, map dream state, then output a structured
  Product Brief that feeds directly into /pm:prd-new.
---
...
1. Extract feature name from `$ARGUMENTS` (strip `--mode=*`). Must be non-empty kebab-case (`^[a-z0-9][a-z0-9-]*[a-z0-9]$`).
