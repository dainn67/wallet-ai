...
task_count=$(ls .claude/epics/$ARGUMENTS/[0-9]*.md 2>/dev/null | grep -cv epic.md)
[ "$task_count" -eq 0 ] && { echo "❌ No tasks to sync. Run: /pm:epic-decompose $ARGUMENTS"; exit 1; }

# 3. Check if already synced (idempotency)
if [ -f ".claude/epics/$ARGUMENTS/github-mapping.md" ]; then
  echo "⚠️ Epic already synced to GitHub. Mapping file exists."
  echo "To re-sync, first delete: .claude/epics/$ARGUMENTS/github-mapping.md"
  exit 0
fi
...
Read `.claude/epics/$ARGUMENTS/epic.md` with the Read tool. Then use the Write tool to update:
...
✅ Synced to GitHub
  ...
  - Mapping: .claude/epics/$ARGUMENTS/github-mapping.md
