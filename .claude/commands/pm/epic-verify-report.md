...
# Prefer final report
REPORT=$(ls -t .claude/context/verify/epic-reports/${EPIC_NAME}-final-*.md 2>/dev/null | head -1)
...
If no report found:
```
❌ No verification report found for epic '$ARGUMENTS'.
   Run: /pm:epic-verify-a $ARGUMENTS to generate a Phase A report.
```
