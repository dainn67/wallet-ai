...
2. **Locate PRD:**
   - If `.claude/prds/$ARGUMENTS.md` doesn't exist → `❌ PRD not found: .claude/prds/$ARGUMENTS.md. Run: /pm:prd-new $ARGUMENTS` and stop.
3. **PRD quality gate:**
...
4. **Check existing epic:**
   - If `.claude/epics/$ARGUMENTS/epic.md` exists → ask `⚠️ Epic '$ARGUMENTS' already exists. Overwrite? (yes/no)`
   - If no → stop.
...
6. **Ensure directory:** `mkdir -p .claude/epics/$ARGUMENTS 2>/dev/null`
...
**Only if** `.claude/designs/$ARGUMENTS/` directory exists:
- Read `.claude/designs/$ARGUMENTS/design-system.md` if it exists — store as design context (color palette, typography, spacing scale, component patterns) for epic generation
- If `.claude/designs/$ARGUMENTS/specs/` directory has files — list spec filenames for task enrichment later
...
Load `.claude/prds/$ARGUMENTS.md` and perform deep analysis:
...
**Decision:** Use design system at `.claude/designs/$ARGUMENTS/design-system.md` as the visual language foundation. All UI tasks reference design tokens instead of hardcoding values.
**Source:** `.claude/designs/$ARGUMENTS/design-system.md`
...
**Only if** screen spec files were found in `.claude/designs/$ARGUMENTS/specs/`:
- For each task that involves UI implementation (task description mentions screens, components, or UI):
  - Add to the task's "What" field: "Reference design spec: `.claude/designs/$ARGUMENTS/specs/{screen}-spec.md`"
...
4. **Next steps:**
   ```
   📋 Next actions:
   → Review epic plan:  /pm:plan-review $ARGUMENTS
