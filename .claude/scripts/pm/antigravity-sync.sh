#!/usr/bin/env bash
# antigravity-sync.sh — Detect gaps and sync features from Claude Code to Antigravity IDE
# Usage:
...
# transform_workflow — Transform a Claude Code command file to Antigravity workflow format
...
sync_all() {
  ...
  if [ -d "$project_root/antigravity" ]; then
    ...
    # Copy workflows and rules from .claude/antigravity/ to root antigravity/
