...
- When two modules don't match → check `.claude/context/active-interfaces.md`
...
- If you change any interface (API, function signature, data format):
  1. Update `.claude/context/active-interfaces.md` with the new interface
  2. `grep` ALL consumers of that interface across the codebase
  3. Update every consumer to match the new interface

### 5. Record Every Fix
- Add an entry to `.claude/context/known-issues.md`:
  ```markdown
...
1. Document the blocker in `.claude/context/known-issues.md`
