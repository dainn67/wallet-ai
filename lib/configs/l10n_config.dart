enum AppLanguage { english, vietnamese }

enum AppCurrency { usd, vnd }

class L10nConfig {
  static const Map<AppLanguage, Map<String, String>> translations = {
    AppLanguage.english: {
      'drawer_records': 'Records',
      'drawer_chat': 'Chat',
      'drawer_categories': 'Categories',
      'reset_all_data': 'Reset All Data',
      'confirm_delete_source': 'Delete this source and all its records?',
      'popup_ok': 'OK',
      'popup_cancel': 'Cancel',
      'popup_confirm': 'Confirm',
      'app_subtitle': 'Personal finance copilot',
      'settings_header': 'Settings',
      'data_management_header': 'Data Management',
      'currency_label': 'Currency',
      'language_label': 'Language',
      'share_app_label': 'Share App',
      'share_app_message': 'Check out Wally AI — your personal finance copilot. Track income and expenses just by chatting!\n\nGoogle Play: {android_url}',
      'currency_change_confirm_title': 'Change Currency',
      'currency_change_confirm_content': 'Changing currency will delete all your records to maintain data consistency. Are you sure?',
      'reset_data_confirm_title': 'Reset All Data',
      'reset_data_confirm_content': 'Are you sure you want to delete all records and reset all balances to zero? This action cannot be undone.',
      'reset_button': 'Reset',
      'chat_hint': "What's your expense?",
      'no_records': 'No records yet',
      'no_records_subtitle': 'Your income and expense records will appear here.',
      'no_categories': 'No categories yet',
      'recent_records': 'Recent Records',
      'income_label': 'Income',
      'spent_label': 'Spent',
      'sources_label': 'Sources',
      'total_balance_label': 'Total Balance',
      'edit_source_title': 'Edit',
      'delete_source_tooltip': 'Delete Source',
      'update_amount_label': 'Update Amount',
      'save_button': 'Save',
      'amount_required_error': 'Amount is required',
      'invalid_amount_error': 'Invalid amount',
      'amount_positive_error': 'Amount must be positive',
      'name_required_error': 'Name is required',
      'description_required_error': 'Description is required',
      'delete_source_confirm_title': 'Delete Source',
      'delete_source_confirm_content':
          'Are you sure you want to delete this source? This will also delete all transaction records associated with this source. This cannot be undone.',
      'delete_record_confirm_title': 'Delete Record',
      'delete_record_confirm_content': 'Are you sure you want to delete this record? This action cannot be undone.',
      'delete_button': 'Delete',
      'dev_mode_enabled': 'Developer mode enabled',
      'dev_mode_disabled': 'Developer mode disabled',
      'add_source_title': 'Add New Source',
      'source_name_label': 'Source Name',
      'initial_amount_label': 'Initial Amount',
      'source_name_hint': 'e.g. Cash, Bank Account',
      'edit_record_title': 'Edit Record',
      'type_label': 'Type',
      'money_source_label': 'Money Source',
      'category_label': 'Category',
      'description_label': 'Description',
      'description_hint': 'Enter description',
      'occurred_at_label': 'Event Time',
      'add_category_title': 'Add Category',
      'edit_category_title': 'Edit Category',
      'category_name_label': 'Category Name',
      'category_name_hint': 'e.g. Shopping, Travel',
      'category_already_exists': 'Category name already exists',
      'emoji_only_input_error': 'Only emoji is allowed',
      'parent_category_label': 'Parent Category',
      'parent_top_level': 'None (top-level)',
      'reparent_cascade_hint': '{count} sub-categories will also move under the new parent.',
      'reminders_label': 'Reminders',
      'reminders_permission_denied_title': 'Notifications are off',
      'reminders_permission_denied_content': 'Open system settings to allow notifications, then reopen Wally.',
      'open_settings_button': 'Open Settings',
      'notif_day1_title': "What did you spend on today?",
      'notif_day1_body': "Let me know and I'll sort it out — takes a sec.",
      'notif_day3_title': "Catch me up on the week so far?",
      'notif_day3_body': "It's been a few days. I can still file them to the right date.",
      'notif_day5_title': "Five days of quiet",
      'notif_day5_body': "If you've spent anything, drop it in. I'll figure out the dates.",
      'notif_day7_title': "Been a week — all good?",
      'notif_day7_body': "No worries if you've been off. I'm here when you want to log.",
      'category_default_label': 'Default',
      'delete_category_confirm_title': 'Delete Category',
      'delete_category_confirm_content': 'Deleting this category will move {count} records to Uncategorized. Continue?',
      'add_sub_category': 'Add Sub-category',
      'chat_thinking': 'Thinking...',
      'today_label': 'Today',
      'yesterday_label': 'Yesterday',
      'max_images_hint': 'Max {count} images',
      'onboarding_slide_1_text': 'Just chat and the AI handles your tracking automatically.',
      'onboarding_slide_2_text': 'View and manage all your income and expense records easily.',
      'onboarding_slide_3_text': 'Change language and currency anytime from the menu drawer.',
      'onboarding_next': 'Next',
      'onboarding_got_it': 'Got it',
      'transfer_source_tooltip': 'Transfer to another source',
      'transfer_title': 'Transfer',
      'transfer_from_label': 'From',
      'transfer_to_label': 'To',
      'transfer_amount_label': 'Amount',
      'transfer_note_label': 'Note',
      'transfer_button': 'Transfer',
      'transfer_select_to_hint': 'Select destination',
      'transfer_select_destination_error': 'Please select a destination',
      'transfer_no_other_source_error': 'No other source to transfer to',
      'transfer_default_description': 'Transfer',
    },
    AppLanguage.vietnamese: {
      'drawer_records': 'Ghi chép',
      'drawer_chat': 'Trò chuyện',
      'drawer_categories': 'Danh mục',
      'reset_all_data': 'Đặt lại tất cả dữ liệu',
      'confirm_delete_source': 'Xóa nguồn này và tất cả hồ sơ của nó?',
      'popup_ok': 'OK',
      'popup_cancel': 'Hủy',
      'popup_confirm': 'Xác nhận',
      'app_subtitle': 'Trợ lý tài chính cá nhân',
      'settings_header': 'Cài đặt',
      'data_management_header': 'Quản lý dữ liệu',
      'currency_label': 'Tiền tệ',
      'language_label': 'Ngôn ngữ',
      'share_app_label': 'Chia sẻ ứng dụng',
      'share_app_message': 'Khám phá Wally AI — trợ lý tài chính cá nhân. Ghi chép thu chi chỉ bằng cách trò chuyện!\n\nGoogle Play: {android_url}',
      'currency_change_confirm_title': 'Thay đổi tiền tệ',
      'currency_change_confirm_content': 'Thay đổi tiền tệ sẽ xóa tất cả hồ sơ của bạn để đảm bảo tính nhất quán của dữ liệu. Bạn có chắc không?',
      'reset_data_confirm_title': 'Đặt lại tất cả dữ liệu',
      'reset_data_confirm_content': 'Bạn có chắc chắn muốn xóa tất cả hồ sơ và đặt lại tất cả số dư về không? Hành động này không thể hoàn tác.',
      'reset_button': 'Đặt lại',
      'chat_hint': 'Bạn đã chi tiêu bao nhiêu?',
      'no_records': 'Chưa có hồ sơ nào',
      'no_records_subtitle': 'Hồ sơ thu nhập và chi tiêu của bạn sẽ xuất hiện ở đây.',
      'no_categories': 'Chưa có danh mục nào',
      'recent_records': 'Hồ sơ gần đây',
      'income_label': 'Thu nhập',
      'spent_label': 'Đã chi',
      'sources_label': 'Nguồn tiền',
      'total_balance_label': 'Tổng số dư',
      'edit_source_title': 'Chỉnh sửa',
      'delete_source_tooltip': 'Xóa nguồn tiền',
      'update_amount_label': 'Cập nhật số tiền',
      'save_button': 'Lưu',
      'amount_required_error': 'Vui lòng nhập số tiền',
      'invalid_amount_error': 'Số tiền không hợp lệ',
      'amount_positive_error': 'Số tiền phải là số dương',
      'name_required_error': 'Vui lòng nhập tên',
      'description_required_error': 'Vui lòng nhập mô tả',
      'delete_source_confirm_title': 'Xóa nguồn tiền',
      'delete_source_confirm_content': 'Bạn có chắc chắn muốn xóa nguồn này không? Hành động này cũng sẽ xóa tất cả hồ sơ giao dịch liên quan. Không thể hoàn tác.',
      'delete_record_confirm_title': 'Xóa hồ sơ',
      'delete_record_confirm_content': 'Bạn có chắc chắn muốn xóa hồ sơ này không? Hành động này không thể hoàn tác.',
      'delete_button': 'Xóa',
      'dev_mode_enabled': 'Đã bật chế độ nhà phát triển',
      'dev_mode_disabled': 'Đã tắt chế độ nhà phát triển',
      'add_source_title': 'Thêm nguồn tiền mới',
      'source_name_label': 'Tên nguồn tiền',
      'initial_amount_label': 'Số dư ban đầu',
      'source_name_hint': 'Ví dụ: Tiền mặt, Tài khoản ngân hàng',
      'edit_record_title': 'Chỉnh sửa hồ sơ',
      'type_label': 'Loại',
      'money_source_label': 'Nguồn tiền',
      'category_label': 'Danh mục',
      'description_label': 'Mô tả',
      'description_hint': 'Nhập mô tả',
      'occurred_at_label': 'Thời điểm phát sinh',
      'add_category_title': 'Thêm danh mục',
      'edit_category_title': 'Chỉnh sửa danh mục',
      'category_name_label': 'Tên danh mục',
      'category_name_hint': 'Ví dụ: Mua sắm, Du lịch',
      'category_already_exists': 'Tên danh mục đã tồn tại',
      'emoji_only_input_error': 'Chỉ cho phép nhập emoji',
      'parent_category_label': 'Danh mục cha',
      'parent_top_level': 'Không có (danh mục cha)',
      'reparent_cascade_hint': '{count} danh mục con cũng sẽ chuyển sang danh mục cha mới.',
      'reminders_label': 'Nhắc nhở',
      'reminders_permission_denied_title': 'Thông báo đang tắt',
      'reminders_permission_denied_content': 'Mở cài đặt hệ thống để cho phép thông báo, rồi mở lại Wally.',
      'open_settings_button': 'Mở cài đặt',
      'notif_day1_title': 'Hôm nay bạn chi gì rồi?',
      'notif_day1_body': 'Nói mình nghe, mình ghi cho — nhanh thôi.',
      'notif_day3_title': 'Cập nhật vài ngày qua nhé?',
      'notif_day3_body': 'Mấy ngày rồi đó. Cứ kể, mình vẫn xếp đúng ngày được.',
      'notif_day5_title': 'Năm ngày rồi không thấy ghi',
      'notif_day5_body': 'Có chi gì cứ kể nha, mình lo phần sắp xếp.',
      'notif_day7_title': 'Đã một tuần — bạn ổn không?',
      'notif_day7_body': 'Không sao nếu bạn bận. Khi nào rảnh thì quay lại nhé.',
      'category_default_label': 'Mặc định',
      'delete_category_confirm_title': 'Xóa danh mục',
      'delete_category_confirm_content': 'Xóa danh mục này sẽ chuyển {count} giao dịch về Chưa phân loại. Tiếp tục?',
      'add_sub_category': 'Thêm danh mục con',
      'chat_thinking': 'Đang suy nghĩ...',
      'today_label': 'Hôm nay',
      'yesterday_label': 'Hôm qua',
      'max_images_hint': 'Tối đa {count} ảnh',
      'onboarding_slide_1_text': 'Chỉ cần nhắn tin, AI sẽ tự động ghi lại giao dịch cho bạn.',
      'onboarding_slide_2_text': 'Xem và quản lý toàn bộ thu chi một cách dễ dàng.',
      'onboarding_slide_3_text': 'Thay đổi ngôn ngữ và tiền tệ bất cứ lúc nào trong menu.',
      'onboarding_next': 'Tiếp theo',
      'onboarding_got_it': 'Bắt đầu',
      'transfer_source_tooltip': 'Chuyển sang nguồn khác',
      'transfer_title': 'Chuyển khoản',
      'transfer_from_label': 'Từ',
      'transfer_to_label': 'Đến',
      'transfer_amount_label': 'Số tiền',
      'transfer_note_label': 'Ghi chú',
      'transfer_button': 'Chuyển',
      'transfer_select_to_hint': 'Chọn nguồn đích',
      'transfer_select_destination_error': 'Vui lòng chọn nguồn đích',
      'transfer_no_other_source_error': 'Không có nguồn nào khác để chuyển',
      'transfer_default_description': 'Chuyển khoản',
    },
  };

  static const Map<AppCurrency, String> currencySymbols = {AppCurrency.usd: r'$', AppCurrency.vnd: '₫'};

  static const Map<AppCurrency, String> currencyCodes = {AppCurrency.usd: 'USD', AppCurrency.vnd: 'VND'};
}
